...
// import './style.css'
    ...
    const emailError = document.querySelector("#emailError");
      ...
      // If the event listeners were already attached, skip re-attaching
    if (!signupFormListenerAttached) {
        signupFormListenerAttached = true;
        ...
        const password_confirm = document.querySelector(
            ...
            alert("Sign-Up failed. Please ensure email isn't already in use and ensure it follows style of 'email@provider.com'");
            return;
            }
    ...
    if (validateEmail(email)) {
        ...
        fetch('http://localhost:8080/passwords/forgot', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    resetPassButton.addEventListener("click", async (e) => {
        ...
        const password_confirm = document.querySelector("#New_Password_confirm").value;
        ...
        if (!validatePassword(password, password_confirm, passwordError)) return;
      
        try {
          const res = await fetch("http://localhost:8080/passwords/reset", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          ...
          if (!res.ok) {
            // ‼️ server rejected the request – keep the form open
            // (optional) show error feedback from the server:
          ...
          // network / other error – keep the form open
